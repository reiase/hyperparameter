...
class Net(nn.Module):
    def __init__(self):
        with param_scope() as hp:
            ...
            self.conv2 = nn.Conv2d(32, 64, hp().cnn.kernel_size(3), 1)
            self.dropout1 = nn.Dropout(hp().dropout1(0.25))
            self.dropout2 = nn.Dropout(hp().dropout1(0.5))
            self.fc1 = nn.Linear(hp().cnn.output_size(9216), hp().fc1(128))
...
def test(model, device, test_loader):
    ...
    mlflow.log_metric("accuracy", correct / len(test_loader.dataset))
    print(
