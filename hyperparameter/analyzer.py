...
class HyperparameterAnalyzer:
    ...
    def analyze_package(self, package_name: str, include_deps: bool = False) -> AnalysisResult:
        """分析一个 Python 包
                ...
                if requires:
                    for req in requires:
                        ...
                        if self._uses_hyperparameter(dep_name):
                            dep_result = self.analyze_package(dep_name, include_deps=False)
                        ...
                        if loc_path.exists():
                            ...
                            for py_file in list(loc_path.rglob("*.py"))[:10]:
                                try:
                                    ...
                                    if "hyperparameter" in content or "param_scope" in content:
                                        return True
                                except Exception:
                                    pass
                # 检查单文件模块
                ...
                try:
                    ...
                    if param_count > 0 or result.functions:
                        hp_packages.append({
                            ...
                            "location": str(dist._path) if hasattr(dist, "_path") else "?",
            ...
            for ns in sorted(by_namespace.keys()):
                ...
                for func in funcs:
                    ...
                    for param in func.params:
                        default_str = f" = {param.default!r}" if param.default is not None else ""
    ...
    def _format_markdown(self, result: AnalysisResult) -> str:
        ...
        if result.functions:
            ...
            for func in result.functions:
                ...
                lines.append(f"| `{func.namespace}` | `{func.name}` | {rel_file}:{func.line} | {params} |")
            lines.append("")
        ...
        def to_dict(obj):
            if isinstance(obj, AnalysisResult):
                return {
                    ...
                    "dependencies": {k: to_dict(v) for k, v in obj.dependencies.items()},
                }
...
class _ASTAnalyzer(ast.NodeVisitor):
    ...
    def _add_function_info(self, node, namespace: Optional[str], is_class: bool = False) -> None:
        """添加函数/类信息"""
        name = node.name
...
def _collect_params(result: AnalysisResult, include_deps: bool = False) -> Dict[str, Dict[str, Any]]:
    """收集所有参数信息
...
""",
    ...
    list_parser.add_argument("package", nargs="?", default=None, 
                            help="包名或路径（不指定则列出所有 hp 包）")
    
    scope_group = list_parser.add_mutually_exclusive_group()
    scope_group.add_argument("--all", "-a", action="store_true", 
                            help="包含依赖包的超参数")
    scope_group.add_argument("--deps", "-d", action="store_true",
                            help="只显示依赖包的超参数")
    scope_group.add_argument("--self", "-s", action="store_true", default=True,
                            help="只显示自身的超参数（默认）")
    
    list_parser.add_argument("--tree", "-t", action="store_true", help="树状显示")
    list_parser.add_argument("--format", "-f", choices=["text", "json", "markdown"],
                            default="text", help="输出格式")
    list_parser.add_argument("--output", "-o", help="输出文件")
    list_parser.add_argument("--verbose", "-v", action="store_true", help="详细输出")
    
    # describe/desc 命令
    desc_parser = subparsers.add_parser("describe", aliases=["desc"], 
                                        help="查看超参数详情")
    ...
    if args.command in ("list", "ls"):
        ...
        if args.deps:
            all_params = {k: v for k, v in all_params.items() 
                         if v.get("source") != result.package}
