...
use hyperparameter::{with_params, get_param, GetOrElse, with_current_storage};
...
where
    T: Into<hyperparameter::Value> + TryFrom<hyperparameter::Value> + for<'a> TryFrom<&'a hyperparameter::Value>,
{
...
fn recursive_test_inner(depth: usize, max_depth: usize, test_id: u64) -> std::pin::Pin<Box<dyn std::future::Future<Output = i64> + Send>> {
    Box::pin(async move {
        if depth >= max_depth {
            // 到达最大深度，读取参数（应该是上一层的 depth，即 max_depth - 1）
            let val: i64 = get_param_dynamic("test_key", -1);
            if max_depth > 0 {
                assert_eq!(val, (max_depth - 1) as i64, "最大深度时参数应该是 {}", max_depth - 1);
            } else {
...
async fn test_random_depth_2() {
    ...
    let result = recursive_test_inner(0, depth, test_id).await;
    let expected = if depth > 1 { (depth - 2) * (depth - 1) / 2 } else { 0 };
...
fn recursive_override_test(depth: usize, max_depth: usize, test_id: u64) -> std::pin::Pin<Box<dyn std::future::Future<Output = Vec<i64>> + Send>> {
    Box::pin(async move {
        if depth >= max_depth {
...
async fn test_override_3() {
    ...
    for (i, &val) in results.iter().enumerate() {
        assert_eq!(val, (depth - 1 - i) as i64, "位置 {} 的值应该是 {}", i, depth - 1 - i);
    }
}
...
fn recursive_multi_param_test(
    ...
    Box::pin(async move {
        if depth >= max_depth {
            ...
            assert_eq!(int_val, prev_depth, "最大深度时 int 参数应该是 {}", prev_depth);
            assert!((float_val - prev_depth as f64 * 1.5).abs() < 1e-10, "最大深度时 float 参数应该是 {}", prev_depth as f64 * 1.5);
            assert_eq!(str_val, format!("depth_{}", max_depth - 1), "最大深度时 str 参数应该是 depth_{}", max_depth - 1);
...
async fn test_multi_param_3() {
    ...
    // max_depth=depth 时，读取到的应该是 depth-1
    let expected_int = if depth > 0 { (depth - 1) as i64 } else { -1 };
    let expected_float = if depth > 0 { (depth - 1) as f64 * 1.5 } else { -1.0 };
    assert_eq!(int_val, expected_int);
...
/// 逻辑：每层设置 param = depth，进入下一层后校验获取到的 param 是否为上一层的 depth
fn recursive_async_test(depth: usize, max_depth: usize, test_id: u64) -> std::pin::Pin<Box<dyn std::future::Future<Output = i64> + Send>> {
    Box::pin(async move {
        if depth >= max_depth {
...
async fn test_concurrent_recursive_1() {
    ...
    let handles: Vec<_> = (0..5)
        .map(|task_id| {
            tokio::spawn(concurrent_recursive_test(0, depth, test_id, task_id))
        })
        .collect();
...
fn mixed_scenario_test(depth: usize, max_depth: usize, test_id: u64) -> std::pin::Pin<Box<dyn std::future::Future<Output = (i64, f64)> + Send>> {
    Box::pin(async move {
        if depth >= max_depth {
            ...
            assert_eq!(int_val, expected_int, "最大深度时 int 参数应该是 {}", expected_int);
            assert!((float_val - expected_float).abs() < 1e-10, "最大深度时 float 参数应该是 {}", expected_float);
...
async fn test_mixed_scenario_2() {
    ...
    let expected_float = if depth > 0 { (depth - 1) as f64 * 3.14 } else { 0.0 };
...
fn deep_nested_restore_test(depth: usize, max_depth: usize, test_id: u64) -> std::pin::Pin<Box<dyn std::future::Future<Output = Vec<i64>> + Send>> {
    Box::pin(async move {
        if depth >= max_depth {
            vec![]
        } else {
...
fn complex_expression_test(depth: usize, max_depth: usize, test_id: u64) -> std::pin::Pin<Box<dyn std::future::Future<Output = i64> + Send>> {
    Box::pin(async move {
        if depth >= max_depth {
            // 到达最大深度，读取参数（应该是上一层的值，即 max_depth - 1）
            let prev_depth = max_depth - 1;
            ...
            assert_eq!(base, expected_base, "最大深度时 base 应该是 {}", expected_base);
            assert_eq!(mult, expected_mult, "最大深度时 mult 应该是 {}", expected_mult);
