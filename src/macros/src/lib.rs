            ...
            syn::Expr::Call(call) => {
                if let syn::Expr::Path(path) = &*call.func {
                    ...
                    let async_func_patterns = [
                        "fetch", "request", "send", "receive", "connect", "listen", "accept",
                        "timeout", "sleep", "delay", "wait", "download", "upload", "load", "save",
                        "read", "write", "get", "post", "put", "delete", "async", "await",
                        "future",
                    ];
...
fn generate_async(items: &[BlockItem], hp: &TokenStream2) -> TokenStream2 {
    ...
    let should_auto_await = last_expr
        .as_ref()
        .map(|e| {
